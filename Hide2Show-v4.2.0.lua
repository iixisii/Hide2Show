--[[
	...
	Description: [
		...
		If you are interested in learning more about the script or obtaining it, you can visit the following link: https://github.com/iixisii/Hide2Show
	]
]]
obs = obslua
__settings__ = nil
prset_gnames_ls = {
	...
	{name = "left to right"; id = "lr"},
	{name = "right to left"; id = "rl"},
	{name = "top to bottom"; id = "tb"},
	{name = "bottom to top"; id = "bt"}
};APP = {
...
function script_defaults(settings_)
	...
	-- [[ Animation defaults ]]
		obs.obs_data_set_default_string(settings_,"action_type_list","<none>");
...
function script_unload() --[[ reset everything to default! ]]
	
end
function welcomeIndex()
	return [[<center><h1 style = "color:#eee;padding:0;margin:0">HIDE & SHOW</h3><h5 style = 'color:#555'><i>V.4 - Made by iixisii</i></h5></center>
		
		<center>
			<p>You can learn more about this script <a href = 'https://github.com/iixisii/Hide2Show'>here</a> or watch a tutorial <a href = 'https://youtu.be/kZeTWPo_ay0'>video</a></p>
		</center>
		<hr/>
	]]
						...
						if list and #list > 0 then
							...
							elseif results and (view_filter == "<none>" or view_filter == "<act>") then -- others view;
								...
								if list[2] == "hide" or list[2] == "show" then
									...
									local act_type_index = 6;if #list == 8 then
										act_type_index = 7; if not list[act_type_index] then
											act_type_index = 6
										end
									end
									if string.lower(strTrim(list[act_type_index])) ~= "def" then
										local at_type_speed = list[act_type_index + 1]
										local at_type = "";if(string.lower(strTrim(list[act_type_index]))=="rl") then
											at_type = "Right → Left";
										elseif string.lower(strTrim(list[act_type_index])) == "lr" then
											at_type = "Left → Right";
										elseif string.lower(strTrim(list[act_type_index])) == "tb" then
											at_type = "Top → Bottom";
										elseif string.lower(strTrim(list[act_type_index])) == "bt" then
											at_type = "Bottom → Top"
										else
											at_type = "<Invalid>"
										end
										place = place .. "<div style = 'margin-left:40px'><span style = 'color:gray'>Animation:</span> <b style = 'color:pink'>" .. at_type .. "</b></div>";
										place = place .. "<div style = 'margin-left:40px'><span style = 'color:gray'>Speed:</span> <b style = 'color:red'>" .. at_type_speed .. " pixels</b></div>";
									end
								end
							end
						end
...
function ActionSetup(properties, property, settings_)
	...
	-- [[ Action Type ]]
		obs.obs_data_set_string(settings_,"action_type_list","<none>");
		obs.obs_data_set_int(settings_,"action_type_speed",10);
		-- dropdown option selection
		local action_type_list = obs.obs_properties_get(properties, "action_type_list");
		if(not action_type_list) then
			action_type_list = obs.obs_properties_add_list(actionOp, "action_type_list", "Animation:", obs.OBS_COMBO_TYPE_LIST, obs.OBS_COMBO_FORMAT_STRING);
			local action_type_speed = obs.obs_properties_add_int(actionOp, "action_type_speed", "Animation Speed:",10,10000000,1);
			obs.obs_property_set_visible(action_type_speed, false)
			obs.obs_property_list_add_string(action_type_list, "None (default)", "<none>");
			-- init action type
			for _, at in ipairs(hns_at_list) do
				obs.obs_property_list_add_string(action_type_list, at.name, at.id);
			end
			obs.obs_property_set_modified_callback(action_type_list, function(properties, property, settings_)
				
				local act_type = obs.obs_data_get_string(settings_, "action_type_list")
				local action_type_speed = obs.obs_properties_get(properties, "action_type_speed");
				if(act_type == "<none>") then
					obs.obs_property_set_visible(action_type_speed, false)
				else
					obs.obs_property_set_visible(action_type_speed, true)
				end
				return true;
			end)
		end
	...
	-- [[ Execute ]]
		...
		if not action_setup_btn then
			...
			action_setup_btn = obs.obs_properties_add_button(actionOp, "action_setup_btn", "Execute", function(properties, property)
				...
				local at_type = obs.obs_data_get_string(settings_, "action_type_list")
				local at_type_speed = tostring(obs.obs_data_get_int(settings_, "action_type_speed"))
				local rp = obs.obs_data_get_string(settings_, "action_setup_group_repeat")
				...
				local insert_act = 0
				if not at_type or at_type == "<none>" then
					at_type = "def"
				end
				-- insert new action (hide)
				if INSERT_INTO_OBS_ARRAY__(action_sct,
				"<act>|(hide)|"..strTrim(source_target).."|".. action_tmt_hide .. "|" .. rp_act .. "|" .. at_type .. "|" .. at_type_speed) then
				...
				if INSERT_INTO_OBS_ARRAY__(action_sct,
				"<act>|(show)|"..strTrim(source_target).."|".. action_tmt_show .. "|" .. rp_act .. "|" .. at_type .. "|" .. at_type_speed) then
					insert_act = insert_act + 1
				else -- error
					obs.obs_property_set_visible(action_setup_error,  true)
					obs.obs_property_set_description(action_setup_error, "(Execute) unable to execute action at this time; please try again!")
			...
			for i = 0, len - 1 do
				...
				if p_item ~= nil then
					...
					if list ~= nil and list[1] == "<act>" then
						...
						local act_type_speed = list[act_type_index + 1]
						local act_type = string.lower(strTrim(list[act_type_index]))
						...
						local ActT = {
							...
							}; format = act_type; speed = tonumber(act_type_speed)
						}
...
local TimeEnd = 0
function InitActionMode(sceneItemName, format, speed, ActionMode, ActionCaller)
	if(format == "def") then
		-- default;
		if ActionMode == "hide" then 
			setSourceVisible(sceneItemName, false)
		elseif ActionMode == "show" then
			setSourceVisible(sceneItemName, true);
		end
		if(type(ActionCaller) == "function") then
			return ActionCaller()
		end
		return
	end
	setSourceVisible(sceneItemName, true);
	local scene_item = __GET_SCENE_ITEM__(sceneItemName);
	local sceneItemCropValues = obs.obs_sceneitem_crop()
	obs.obs_sceneitem_get_crop(scene_item.item, sceneItemCropValues)
	local w = obs.obs_source_get_width(scene_item.source);
	local h = obs.obs_source_get_height(scene_item.source);
	local currPos = obs.vec2()
	obs.obs_sceneitem_get_pos(scene_item.item, currPos)
	local init_crop_value = 0; if ActionMode == "show" then
		if(format == "lr" or format == "rl") then
			init_crop_value = w;
		elseif format == "bt" or format == "tb" then
			init_crop_value = h;
		end
	end
	-- [[ LEFT TO RIGHT  & RIGHT TO LEFT HANDLE FUNCTION ]]
		local function InitCropLR_RL()
			if(ActionMode == "hide" and init_crop_value < w) or (ActionMode == "show" and init_crop_value > 0) then
				if ActionMode == "hide" then
					init_crop_value = init_crop_value + speed
					if format == "rl" then
						currPos.x = currPos.x + speed
					end
				elseif ActionMode == "show" then
					init_crop_value = init_crop_value - speed
					if format == "rl" then
				...
				if(format == "lr") then 
					sceneItemCropValues.right = init_crop_value;
				elseif format == "rl" then
					sceneItemCropValues.left = init_crop_value;
				end
				obs.obs_sceneitem_set_crop(scene_item.item,sceneItemCropValues);
				obs.obs_sceneitem_set_pos(scene_item.item, currPos)
			elseif (ActionMode == "hide" and init_crop_value >= w) or (ActionMode == "show" and init_crop_value <= 0) then -- animation done
				if ActionMode == "hide" then 
					setSourceVisible(sceneItemName, false)
				elseif ActionMode == "show" then
					setSourceVisible(sceneItemName, true);
				end
				obs.timer_remove(InitCropLR_RL);
				scene_item.release();
				if(type(ActionCaller) == "function") then
					return ActionCaller()
				end
	...
	-- [[ TOP TO BOTTOM & BOTTOM TO TOP HANDLE FUNCTION ]]
		local function InitCropTB_BT()
			if(ActionMode == "hide" and init_crop_value < h) or (ActionMode == "show" and init_crop_value > 0) then
				if ActionMode == "hide" then
					init_crop_value = init_crop_value + speed
					if format == "tb" then
						currPos.y = currPos.y + speed
					end
				elseif ActionMode == "show" then
					init_crop_value = init_crop_value - speed
					if format == "tb" then
				...
				if(format == "bt") then 
					sceneItemCropValues.bottom = init_crop_value;
				elseif format == "tb" then
					sceneItemCropValues.top = init_crop_value;
				end
				obs.obs_sceneitem_set_crop(scene_item.item,sceneItemCropValues);
				obs.obs_sceneitem_set_pos(scene_item.item, currPos)
			elseif (ActionMode == "hide" and init_crop_value >= h) or (ActionMode == "show" and init_crop_value <= 0) then -- animation done
				if ActionMode == "hide" then 
					setSourceVisible(sceneItemName, false)
				elseif ActionMode == "show" then
					setSourceVisible(sceneItemName, true);
				...
				scene_item.release();
				obs.timer_remove(InitCropTB_BT);
	...
	if(format == "lr" or format == "rl") then
		return obs.timer_add(InitCropLR_RL,1);
	elseif format == "tb" or format == "bt" then
		return obs.timer_add(InitCropTB_BT,1)
	else
		if scene_item then scene_item.release(); end
		return 
	end
end
function main()
			...
			local function initHide()
				...
				-- [[ ALL OR ALL_LIKE OR RAND OR RAND_LIKE ]]
					...
					elseif iter.hide.name == "rand" or (APP_ACTIONS:IsLikeAction(iter.hide.name) and APP_ACTIONS:CmdTar(iter.hide.name) == "rand") then -- {RAND, RAND_LIKE}
						...
						InitActionMode(iter.hide.name, iter.hide.format,iter.hide.speed, "hide", function()
							iter.hide.isHidden = true
							iter.show.isShown = false
							iter.hide.time.tick = 0
							iter.show.time.tick = os.clock() * 1000
		...
		elseif iter.hide and iter.hide.isHidden and iter.show and not iter.show.isShown then -- (Show)
			local function initShow()
				...
				-- [[ ALL OR ALL_LIKE OR RAND OR RAND_LIKE ]]
					...
					else -- default (show) action
						InitActionMode(iter.show.name, iter.show.format,iter.show.speed, "show", function()
							iter.hide.isHidden = false
							iter.show.isShown = true
							iter.hide.time.tick = os.clock() * 1000
							iter.show.time.tick = 0
